...
def train_ccs_probe(
    ...
    *,
    ...
    for group in np.unique(groups):
        ...
        indices = sorted(
            [
                group_labels.tolist().index(True),
                group_labels.tolist().index(False),
            ]
        )
        activations_1.append(group_activations[indices[0]])
        activations_2.append(group_activations[indices[1]])
    activations_1 = torch.tensor(np.array(activations_1)).to(
        dtype=torch.float32
    )
    activations_2 = torch.tensor(np.array(activations_2)).to(
        dtype=torch.float32
    )
    ...
    def get_loss():
        ...
        loss_l2 = (
            sum(param.norm() ** 2 for param in probe.parameters())
            * config.weight_decay
        )
